...
from Ramezanee_SUT_Task1.models.net import get_model
from Ramezanee_SUT_Task1.models.multi_device_model import MultiDeviceModelContainer
...
class Baseline(torch.nn.Module):
    ...
    def __init__(self, config: Config):
        ...
        self.resampler = torchaudio.transforms.Resample(orig_freq=44100, new_freq=config.sample_rate)
        ...
        self.model = MultiDeviceModelContainer(base_model, devices=config.device_ids, submission=4)
        self.model.eval()
...
def load_model(model_file_path: Optional[str] = None) -> Baseline:
    ...
    if model_file_path != None:
        ...
        model.model.base_model.load_state_dict(state_dict, strict=True)
    else:
        # Define the base resource directory
        ckpts_dir = files(ckpts)
        # Use `as_file` to ensure we get a real temporary path (if needed)
        with as_file(ckpts_dir.joinpath("GLOBAL.pth")) as global_path:
            # Load checkpoint to CPU (compatible with CPU inference)
            ckpt = torch.load(str(global_path), map_location="cpu")
            # Handle Lightning-style checkpoints with nested 'state_dict'
            state_dict = ckpt['model_state_dict']
            new_state_dict = {}
            ...
            model.model.base_model.load_state_dict(new_state_dict, strict=True)
